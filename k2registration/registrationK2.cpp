...
int Registration::getbigfiltermapsize() const
{
 ...
 */
bool Registration::apply3(const Frame *rgb, const Frame *depth, Frame *undistorted, Frame *registered, const bool enable_filter, Frame *bigdepth) const
{
  ...
  rgbt *registered_data = (rgbt*)registered->data;
  ...
  if(enable_filter){
    ...
    for(int i = 0; i < size_depth; ++i, ++map_c_off, ++undistorted_data, ++registered_data){
      ...
      *registered_data = (z - min_z) / z > filter_tolerance ? rgb_t({0,0,0}) : *(rgb_data + c_off);
    }
  ...
  else
  {
    ...
    for(int i = 0; i < size_depth; ++i, ++map_c_off, ++registered_data){
      ...
      // check if offset is out of image
      if(c_off >= 0)
        *registered_data = c_off < 0 ?rgb_t({0,0,0}) : *(rgb_data + c_off);
    }
  }
  return true;
}
...
Registration::Registration(const Freenect2Device::IrCameraParams &depth_p,const  Freenect2Device::ColorCameraParams &rgb_p):
